...
######################
setwd('G:\\YouRui')
pbmc_CD8T <- readRDS('G:\\YouRui\\CD8Tcell\\CD8T.rds')

##################################################### Subcluster
Idents(pbmc_CD8T) <- pbmc_CD8T$cell                                       
pbmc_PC <- subset(pbmc_CD8T, idents = c('Tex_IFIT1','CTL_CLNK','Tem_SLC4A10'))                                       
rm(pbmc_CD8T)
gc()
########################
coembed <- pbmc_PC
...
df <- coembed@meta.data %>%
    as.data.frame() %>%
    subset(., cell %in% c("Tem_SLC4A10", "CTL_CLNK", "Tex_IFIT1"))

coembed <- coembed[, rownames(df)]
cols <- ArchR::paletteDiscrete(unique(coembed@meta.data[, "cell"]))
###################################
traj1 <- c("Tem_SLC4A10", "CTL_CLNK", "Tex_IFIT1")
#######################
...
coembed <- AddTrajectory(object = coembed, 
                          trajectory = c("Tem_SLC4A10", "CTL_CLNK", "Tex_IFIT1"),
                          group.by = "cell", 
